...
final class SupportTicketLifecycleProcessor implements Processor {

    private final ObjectMapper objectMapper;
    private final ConcurrentMap<String, TicketRecord> ticketsByConversationId = new ConcurrentHashMap<>();

    SupportTicketLifecycleProcessor(ObjectMapper objectMapper) {
    ...
    private TicketRecord applyAction(String conversationId, TicketRecord existing, TicketAction action, String query) {
        ...
        return switch (action) {
            ...
            case UPDATE -> new TicketRecord(current.ticketId(), "IN_PROGRESS", firstNonBlank(query, current.summary()), current.assignedQueue());
            case CLOSE -> new TicketRecord(current.ticketId(), "CLOSED", firstNonBlank(query, current.summary()), current.assignedQueue());
            case STATUS -> current;
        };
    }

    private TicketRecord createRecord(String conversationId, String query) {
        return new TicketRecord(ticketIdFor(conversationId), "OPEN", firstNonBlank(query, "Customer requested support follow-up"), queueFor(query));
    }
